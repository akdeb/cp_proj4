function hdr_naive = makehdr_naive(ldrs, exposures)
    ...
    for i=1:1:5
       ...
       hdr_naive(:,:,1) = hdr_naive(:,:,1) + double(im_r);
       hdr_naive(:,:,2) = hdr_naive(:,:,2) + double(im_g);
       hdr_naive(:,:,3) = hdr_naive(:,:,3) + double(im_b);
    end
...
%     hdr_naive(:,:,3) = (hdr_naive(:,:,3) - min(min(hdr_naive(:,:,3))))/(max(max(hdr_naive(:,:,3))) - min(min(hdr_naive(:,:,3))));
    ...
    figure(1), imshow(tonemap(hdr_naive))
    figure(2), imshow(tonemap(log(hdr_naive)))
end
